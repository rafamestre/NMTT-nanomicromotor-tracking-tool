...
This code was written to be compatible Python 3.3+, as well
...
fileName = Path(easygui.fileopenbox(default=default))
...
# initialFrame = cv2.addWeighted(initialFrame, contrast, initialFrame, 0, 0)
if not ok:
...
for bbox in bounding_box_list[0]:
    ...
    multi_tracker.add(generate_tracker(tracker_type), frameResized, tuple(bbox))
...
#Other useful lists
#KeepList tells you which IDs are kept in the next frame, i.e. which particles
#where not lost.

keepList = [True for i in ids]
ID_array = [ids.copy()]
...
while True:
    ...
    if displayScaleBar:
        ...
        if displayScaleBarText:
            if f != 1:
                cv2.putText(frameResized,str(scaleNumber) + ' um',(int(f*width*0.7),int(f*height*0.06)+generalOffset),cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255,255,255), 2)
